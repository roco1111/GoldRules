...
import com.rosario.hp.goldrules.adapter.ExpandableListAdapter;
import com.rosario.hp.goldrules.containers.CustomConstants;
import com.rosario.hp.goldrules.datastore.Singleton;
import com.rosario.hp.goldrules.processes.ScanHelper;
import com.rosario.hp.goldrules.tasks.MonitorTask;
import com.rosario.hp.goldrules.tasks.MonitorTask.OnBeaconDataChangedListener;
import com.rosario.hp.goldrules.utils.BluetoothHelper;
import com.rosario.hp.goldrules.utils.Logger;
import com.rosario.hp.goldrules.utils.ToastUtils;

import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

public class MainActivityBeacon extends BaseActivity implements OnChildClickListener, OnCheckedChangeListener{
                    ...
                    new String[]{Manifest.permission.ACCESS_COARSE_LOCATION}, new PermissionsResultAction() {
                        ...
                        public void onGranted() {
                            ...
                            if (!mBluetoothHelper.isBluetoothLeSupported()) {
                                ToastUtils.makeWarningToast(MainActivityBeacon.this, getString(R.string.toast_no_le));
                                buttonView.setChecked(false);
                                return;
                            } else {
                                if (!mBluetoothHelper.isBluetoothOn()) {
                                    ScanHelper.cancelService(MainActivityBeacon.this);
                        ...
                        public void onDenied(final String permission) {
                            ToastUtils.makeWarningToast(MainActivityBeacon.this, getString(R.string.toast_warning_permission_not_granted));
                            buttonView.setChecked(false);
                        }
                    });
        }
    ...
    protected int getLayoutResource() {
        return R.layout.activity_main_beacon;
    }
    ...
    public void serviceToggle() {
        ...
        if (mIsScanRunning) {
            ...
            ScanHelper.scanForIBeacons(MainActivityBeacon.this, getScanTime(), getPauseTime());
        }
    }
    ...
    private void handleResume() {
        ...
        if (mBluetoothHelper.isBluetoothOn()
            ...
            Logger.i(TAG, "Bluetooth ha sido activado");
            if (mIsScanRunning) {
                Logger.d(TAG, "Reinicio del servicio de escaneo");
                ScanHelper.scanForIBeacons(MainActivityBeacon.this, getScanTime(), getPauseTime());
            }
    ...
    protected void removeReceivers() {
        try {
            ...
            Logger.d(TAG, "El receptor de escaneo no se ha registrado correctamente");
        } catch (final Exception e) {
            Logger.d(
                    TAG,
                    "El receptor de escaneo ya no está registrado. Excepción : "
    ...
    protected void stopMonitorTask() {
        if (mMonitorTask != null) {
            Logger.d(TAG, "Tarea de monitor en pausa");
            mMonitorTask.stop();
        }
    }
    ...
    private void registerMonitorTask() {
        Logger.d(TAG, "Iniciar tarea de monitor");
    ...
    private void registerResponseReceiver() {
        Logger.d(TAG, "Registro del receptor de respuestas");
    ...
    private void startPreferencesActivity() {
        Logger.d(TAG, "Inicio de la actividad de configuración");
    ...
    private void updateListData() {
        ...
        Logger.d(TAG, "Actualización de datos de la lista");
        mListDataHeader = new ArrayList<>();
        mListDataHeader.add("Dispositivos disponibles (" + Singleton.getInstance().getAvailableDeviceListSize() + ")");
        mListDataHeader.add("New Devices (" + Singleton.getInstance().getNewDeviceListSize() + ")");
        mListDataHeader.add("Nuevos dispositivos (" + Singleton.getInstance().getUpdatedDeviceListSize() + ")");
        mListDataHeader.add("Mover dispositivos más cercanos (" + Singleton.getInstance().getMovingCloserDeviceListSize() + ")");
        mListDataHeader.add("Moviéndose más lejos del dispositivo (" + Singleton.getInstance().getMovingFartherDeviceListSize() + ")");
        mListDataHeader.add("Dispositivos que desaparecen (" + Singleton.getInstance().getDisappearingDeviceListSize() + ")");
    ...
    public class ResponseReceiver extends BroadcastReceiver {
        ...
        public void onReceive(final Context context, final Intent intent) {
            ...
            if (intent.getAction().equalsIgnoreCase(
                ...
                ToastUtils.makeProgressToast(MainActivityBeacon.this, getString(R.string.toast_scanning));
                ...
                ToastUtils.makeInfoToast(MainActivityBeacon.this, getString(R.string.toast_completed));
            }
        }
    }
}
